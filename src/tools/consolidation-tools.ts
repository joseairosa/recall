...
function getService(): ConsolidationService {
  if (!_service)
    throw new Error(
      "Consolidation service not initialized. Call setConsolidationMemoryStore first.",
    );
  return _service;
}
...
function getStore(): MemoryStore {
  if (!_store)
    throw new Error(
      "Consolidation store not initialized. Call setConsolidationMemoryStore first.",
    );
  return _store;
}
...
/** Exported accessor for use by consolidated tool handlers */
export function getConsolidationStore(): MemoryStore {
  return getStore();
}
    ...
    handler: async (args) => {
      try {
        ...
        if (!needed) {
          return ok({
            needed: false,
            message: "Consolidation not needed at this time.",
          });
        }
      ...
      try {
        ...
        try {
          ...
          if (info.type === "anthropic") {
            providerWarning =
              "Using Anthropic keyword-based embeddings. Consolidation quality may be lower. Consider using OpenAI, Voyage, or Cohere for better results.";
          }
        } catch {}
